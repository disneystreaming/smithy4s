...
trait Writer[-In, +Out, -A] { self =>
  ...
  /**
    * Replaces this writer with a Noop unless a condition is met.
    */
  def when[Message >: Out <: In](condition: Boolean): Writer[Message, Message, A] =
    if (condition) self: Writer[Message, Message, A] else Writer.noop[Message]
...
object Writer {
  ...
  /**
    ...
    */
  def constant[Message](message: Message): Encoder[Message, Any] =
