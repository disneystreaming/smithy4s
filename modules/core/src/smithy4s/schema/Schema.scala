...
object Schema {
  ...
  private [smithy4s] class PartiallyAppliedRequired[S, A](private val schema: Schema[A]) extends AnyVal {
    ...
    def apply(label: String, get: S => A, replace: A => (S => S)): FieldLens[S, A] = Field(label, schema, get, replace)
  }
  ...
  private [smithy4s] class PartiallyAppliedOptional[S, A](private val schema: Schema[A]) extends AnyVal {
    ...
    def apply(label: String, get: S => Option[A], replace: Option[A] => (S => S)): FieldLens[S, Option[A]] = Field.optional(label, schema, get, replace)
  }
