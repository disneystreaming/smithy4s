...
trait SchemaVisitor[F[_]] extends (Schema ~> F){
  ...
  def surject[A, B](schema: Schema[A], tags: List[ShapeTag[_]], to: A => Either[ConstraintError, B], from: B  => A) : F[B]
  ...
  def apply[A](schema: Schema[A]) : F[A] = schema match {
    ...
    case SurjectionSchema(schema, tags, to, from) => surject(schema, tags, to, from)
