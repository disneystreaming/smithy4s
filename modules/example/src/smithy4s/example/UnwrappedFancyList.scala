...
object UnwrappedFancyList extends Newtype[smithy4s.example.refined.FancyList] {
  ...
  val underlyingSchema : smithy4s.Schema[smithy4s.example.refined.FancyList] = list(string).refined[smithy4s.example.refined.FancyList](smithy4s.example.FancyListFormat()).withId(id).addHints(hints)
