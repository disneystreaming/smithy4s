...
trait BrandServiceGen[F[_, _, _, _, _]] {
  ...
  def addBrands(brands: Option[scala.collection.immutable.List[String]] = None) : F[AddBrandsInput, Nothing, Unit, Nothing, Nothing]
  ...
  class Transformed[G[_, _, _, _, _]](transformation : Transformation[F, G]) extends BrandServiceGen[G] {
    def addBrands(brands: Option[scala.collection.immutable.List[String]] = None) = transformation[AddBrandsInput, Nothing, Unit, Nothing, Nothing](self.addBrands(brands))
  }
}
...
object BrandServiceGen extends Service[BrandServiceGen, BrandServiceOperation] {
  ...
  val endpoints: scala.List[Endpoint[BrandServiceOperation, _, _, _, _, _]] = scala.List(
    AddBrands,
  )
  ...
  object reified extends BrandServiceGen[BrandServiceOperation] {
    def addBrands(brands: Option[scala.collection.immutable.List[String]] = None) = AddBrands(AddBrandsInput(brands))
  }
