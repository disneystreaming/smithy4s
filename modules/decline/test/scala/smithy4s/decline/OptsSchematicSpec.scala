...
object OptsSchematicSpec extends SimpleIOSuite {
  ...
  object Recursive extends smithy4s.ShapeTag.Companion[Recursive] {
    ...
    val schema: smithy4s.Schema[Recursive] = recursive(
      struct(
        string.required[Recursive]("name", _.name).addHints(smithy.api.Required()),
  ...
  pureTest("nested struct with required fields") {
    assert.parsed(
      parseOpts(sampleStruct("input", Person.schema))("--name", "Mary", "--age", "35"),
      Person("Mary", 35),
    )
  }
  ...
  pureTest("nested struct with all optional fields - all arguments") {
    assert.parsed(
      parseOpts(sampleStruct("input", FullyOptional.schema))("--value1", "1.1", "--value2", "a"),
  ...
  pureTest("compile document") {
    ...
    assert.parsed(
      ...
      Document.obj(
        ...
        "baz" -> Document.array(Document.fromString("a"), Document.fromString("b")),
      ),
    )
  }
  ...
  pureTest("compile nested list of recursive type") {
    ...
    assert.parsed(
      parseOpts(sampleStruct("recursive", sampleStruct("items", list(Recursive.schema))))(
        "--items",
        """{"name": "foo", "parent": {"name": "bar"}}""",
        "--items",
        """{"name":"baz"}""",
