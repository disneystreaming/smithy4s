...
object UnliftResource {
  ...
  /** Transform a resource with an interpreter to a plain interpreter (allocating and using the
    ...
    */
  def unliftService[Alg[_[_, _, _, _, _]], Op[_, _, _, _, _], F[
      _
  ]: MonadCancelThrow](
    ...
    service.transform[GenLift[F]#λ](unliftResource(algRes, service))
  ...
  /** Transform a resource with an interpreter to a plain interpreter (allocating and using the
    ...
    */
  def unliftResource[Alg[_[_, _, _, _, _]], Op[_, _, _, _, _], F[
      _
  ]: MonadCancelThrow](
      algRes: Resource[F, smithy4s.Monadic[Alg, F]],
      service: Service[Alg, Op]
