...
/** An RPC-based protocol that sends JSON payloads. This protocol does not use
  ...
  */
final case class AwsJson1_0(http: Option[List[String]] = None, eventStreamHttp: Option[List[String]] = None) extends HttpConfiguration
object AwsJson1_0 extends ShapeTag.Companion[AwsJson1_0] {
  val id: ShapeId = ShapeId("aws.protocols", "awsJson1_0")
  ...
  implicit val protocol: smithy4s.Protocol[AwsJson1_0] = new smithy4s.Protocol[AwsJson1_0] {
  ...
  implicit val schema: Schema[AwsJson1_0] = recursive(struct(
    StringList.underlyingSchema.optional[AwsJson1_0]("http", _.http).addHints(smithy.api.Documentation("The priority ordered list of supported HTTP protocol versions.")),
    StringList.underlyingSchema.optional[AwsJson1_0]("eventStreamHttp", _.eventStreamHttp).addHints(smithy.api.Documentation("The priority ordered list of supported HTTP protocol versions that\nare required when using event streams with the service. If not set,\nthis value defaults to the value of the `http` member. Any entry in\n`eventStreamHttp` MUST also appear in `http`.")),
  ){
    AwsJson1_0.apply
