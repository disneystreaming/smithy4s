...
object GetObjectOutput extends ShapeTag.Companion[GetObjectOutput] {
  ...
  val size = ObjectSize.schema.required[GetObjectOutput]("size", _.size, n => c => c.copy(size = n)).addHints(HttpHeader("X-Size"), Required())
  val data = string.optional[GetObjectOutput]("data", _.data, n => c => c.copy(data = n)).addHints(HttpPayload())
