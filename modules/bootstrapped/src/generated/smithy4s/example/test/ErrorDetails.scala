...
object ErrorDetails extends ShapeTag.Companion[ErrorDetails] {
  ...
  implicit val schema: Schema[ErrorDetails] = struct(
    timestamp.required[ErrorDetails]("date", _.date).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "timestampFormat"), smithy4s.Document.fromString("epoch-seconds"))),
