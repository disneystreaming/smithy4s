...
object SayHelloOutput extends ShapeTag.Companion[SayHelloOutput] {
  ...
  implicit val schema: Schema[SayHelloOutput] = struct(
    SayHelloPayload.schema.required[SayHelloOutput]("payload", _.payload).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "httpPayload"), smithy4s.Document.obj())),
    string.required[SayHelloOutput]("header1", _.header1).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "httpHeader"), smithy4s.Document.fromString("X-H1"))),
