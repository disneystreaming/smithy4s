...
object SayHelloOutput extends ShapeTag.Companion[SayHelloOutput] {
  ...
  val payload = SayHelloPayload.schema.required[SayHelloOutput]("payload", _.payload, n => c => c.copy(payload = n)).addHints(HttpPayload(), Required())
  val header1 = string.required[SayHelloOutput]("header1", _.header1, n => c => c.copy(header1 = n)).addHints(HttpHeader("X-H1"), Required())
