...
object ComplexError extends ShapeTag.Companion[ComplexError] {
  ...
  val value: FieldLens[ComplexError, Int] = int.required[ComplexError]("value", _.value, n => c => c.copy(value = n)).addHints(Required())
  val message: FieldLens[ComplexError, String] = string.required[ComplexError]("message", _.message, n => c => c.copy(message = n)).addHints(Required())
  val details: FieldLens[ComplexError, Option[ErrorDetails]] = ErrorDetails.schema.optional[ComplexError]("details", _.details, n => c => c.copy(details = n))
