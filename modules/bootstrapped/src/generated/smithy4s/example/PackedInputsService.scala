...
object PackedInputsServiceGen extends Service.Mixin[PackedInputsServiceGen, PackedInputsServiceOperation] {
  ...
  implicit final class TransformFunctorOps[F[_]](private val alg: FunctorAlgebra[PackedInputsServiceGen, F]) extends AnyVal {
    def transform: Transformation.PartiallyApplied[FunctorAlgebra[PackedInputsServiceGen, F]] = Transformation.of(alg)
  }

  implicit final class TransformBifunctorOps[F[_, _]](private val alg: BiFunctorAlgebra[PackedInputsServiceGen, F]) extends AnyVal {
    def transform: Transformation.PartiallyApplied[BiFunctorAlgebra[PackedInputsServiceGen, F]] = Transformation.of(alg)
  }

  implicit final class TransformOps[F[_, _, _, _, _]](private val alg: PackedInputsServiceGen[F]) extends AnyVal {
    def transform: Transformation.PartiallyApplied[PackedInputsServiceGen[F]] = Transformation.of(alg)
  }
}
