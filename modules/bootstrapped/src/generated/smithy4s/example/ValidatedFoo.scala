...
object ValidatedFoo extends ShapeTag.Companion[ValidatedFoo] {
  ...
  implicit val schema: Schema[ValidatedFoo] = struct(
    ValidatedString.schema.field[ValidatedFoo]("name", _.name).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "default"), smithy4s.Document.fromString("abc"))),
