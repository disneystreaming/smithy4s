...
object TestStructurePattern extends Newtype[TestStructurePatternTarget] {
  ...
  val hints: Hints = Hints(
    Hints.Binding.DynamicBinding(ShapeId("alloy", "structurePattern"), smithy4s.Document.obj("pattern" -> smithy4s.Document.fromString("{one}-{two}"), "target" -> smithy4s.Document.fromString("smithy4s.example#TestStructurePatternTarget"))),
  ).lazily
