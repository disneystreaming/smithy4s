...
object DefaultNotCapitalized extends ShapeTag.Companion[DefaultNotCapitalized] {
  ...
  implicit val schema: Schema[DefaultNotCapitalized] = struct(
    Username.schema.required[DefaultNotCapitalized]("name", _.name).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "default"), smithy4s.Document.fromString("hello"))),
