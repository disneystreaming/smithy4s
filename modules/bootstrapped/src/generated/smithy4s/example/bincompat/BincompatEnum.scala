...
sealed abstract class BincompatEnum(_name: String, _stringValue: String, _intValue: Int, _hints: Hints) extends Enumeration.Value {
...
object BincompatEnum extends Enumeration[BincompatEnum] with ShapeTag.Companion[BincompatEnum] {
  ...
  implicit val schema: Schema[BincompatEnum] = stringEnumeration(values).withId(id).addHints(hints)
}
