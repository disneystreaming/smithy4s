...
object Person extends ShapeTag.Companion[Person] {
  ...
  implicit val schema: Schema[Person] = struct(
    string.required[Person]("name", _.name).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "httpLabel"), smithy4s.Document.obj())),
    string.optional[Person]("town", _.town).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "httpQuery"), smithy4s.Document.fromString("town"))),
