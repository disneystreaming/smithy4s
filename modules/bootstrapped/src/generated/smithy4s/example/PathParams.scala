...
object PathParams extends ShapeTag.Companion[PathParams] {
  ...
  val int = smithy4s.schema.Schema.int.required[PathParams]("int", _.int).addHints(smithy.api.HttpLabel(), smithy.api.Required())
  ...
  implicit val schema: Schema[PathParams] = struct(
    ...
    PathParams.apply
  }.withId(ShapeId("smithy4s.example", "PathParams")).addHints(hints)
}
