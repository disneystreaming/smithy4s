...
object EnumWithDeprecations extends Enumeration[EnumWithDeprecations] with ShapeTag.Companion[EnumWithDeprecations] {
  ...
  val hints: Hints = Hints(
    ...
    Hints.Binding.DynamicBinding(ShapeId("smithy.api", "documentation"), smithy4s.Document.fromString("some docs here")),
  ).lazily

  @deprecated(message = "N/A", since = "N/A")
  case object OLD extends EnumWithDeprecations("OLD", "OLD", 0, Hints.empty) {
    override val hints: Hints = Hints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "deprecated"), smithy4s.Document.obj())).lazily
  }
