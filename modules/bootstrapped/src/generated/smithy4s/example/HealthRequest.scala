...
object HealthRequest extends ShapeTag.Companion[HealthRequest] {
  ...
  implicit val schema: Schema[HealthRequest] = struct(
    string.validated(smithy.api.Length(min = Some(0L), max = Some(5L))).optional[HealthRequest]("query", _.query).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "httpQuery"), smithy4s.Document.fromString("query"))),
