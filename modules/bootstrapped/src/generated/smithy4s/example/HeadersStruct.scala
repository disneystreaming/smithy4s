...
object HeadersStruct extends ShapeTag.Companion[HeadersStruct] {
  ...
  val int = smithy4s.schema.Schema.int.optional[HeadersStruct]("int", _.int).addHints(smithy.api.HttpHeader("int"))
  ...
  implicit val schema: Schema[HeadersStruct] = struct(
    ...
    HeadersStruct.apply
  }.withId(ShapeId("smithy4s.example", "HeadersStruct")).addHints(hints)
}
