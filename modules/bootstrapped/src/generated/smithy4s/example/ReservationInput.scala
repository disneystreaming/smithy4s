...
object ReservationInput extends ShapeTag.$Companion[ReservationInput] {
  ...
  val name: FieldLens[ReservationInput, String] = string.required[ReservationInput]("name", _.name, n => c => c.copy(name = n)).addHints(HttpLabel(), Required())
  val town: FieldLens[ReservationInput, Option[String]] = string.optional[ReservationInput]("town", _.town, n => c => c.copy(town = n)).addHints(HttpQuery("town"))
