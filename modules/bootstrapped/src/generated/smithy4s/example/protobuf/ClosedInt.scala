...
object ClosedInt extends Enumeration[ClosedInt] with ShapeTag.Companion[ClosedInt] {
  ...
  case object FOO extends ClosedInt("FOO", "FOO", 0, Hints.empty) {
    override val hints: Hints = Hints(Hints.Binding.DynamicBinding(ShapeId("alloy.proto", "protoIndex"), smithy4s.Document.fromDouble(0.0d))).lazily
  }
  case object BAR extends ClosedInt("BAR", "BAR", 1, Hints.empty) {
    override val hints: Hints = Hints(Hints.Binding.DynamicBinding(ShapeId("alloy.proto", "protoIndex"), smithy4s.Document.fromDouble(1.0d))).lazily
  }
