...
object RecursiveInputServiceGen extends Service.Mixin[RecursiveInputServiceGen, RecursiveInputServiceOperation] {
  ...
  final implicit class RecursiveInputServiceGenTransformFunctor[F[_]](private val self: RecursiveInputServiceGen[({ type L[A, B, C, D, E] = F[C] })#L]) extends AnyVal {
    final def transform: Transformation.PartiallyApplied[RecursiveInputServiceGen[({ type L[A, B, C, D, E] = F[C] })#L]] = Transformation.of(self)
  }
  final implicit class RecursiveInputServiceGenTransformBifunctor[F[_, _]](private val self: RecursiveInputServiceGen[({ type L[A, B, C, D, E] = F[B, C] })#L]) extends AnyVal {
    final def transform: Transformation.PartiallyApplied[RecursiveInputServiceGen[({ type L[A, B, C, D, E] = F[B, C] })#L]] = Transformation.of(self)
  }
  final implicit class RecursiveInputServiceGenTransformFull[F[_, _, _, _, _]](private val self: RecursiveInputServiceGen[F]) extends AnyVal {
    final def transform: Transformation.PartiallyApplied[RecursiveInputServiceGen[F]] = Transformation.of(self)
  }
}
