...
object Queries extends ShapeTag.Companion[Queries] {
  ...
  val ts1: FieldLens[Queries, Option[Timestamp]] = timestamp.optional[Queries]("ts1", _.ts1, n => c => c.copy(ts1 = n)).addHints(HttpQuery("ts1"))
  val ts2: FieldLens[Queries, Option[Timestamp]] = timestamp.optional[Queries]("ts2", _.ts2, n => c => c.copy(ts2 = n)).addHints(TimestampFormat.DATE_TIME.widen, HttpQuery("ts2"))
  val ts3: FieldLens[Queries, Option[Timestamp]] = timestamp.optional[Queries]("ts3", _.ts3, n => c => c.copy(ts3 = n)).addHints(TimestampFormat.EPOCH_SECONDS.widen, HttpQuery("ts3"))
