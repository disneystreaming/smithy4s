...
object DynamoDBGen extends Service.Mixin[DynamoDBGen, DynamoDBOperation] {
  ...
  val hints: Hints = Hints(
    ...
    aws.api.Service(sdkId = "DynamoDB", arnNamespace = Some(aws.api.ArnNamespace.unsafeApply("dynamodb")), cloudFormationName = Some(aws.api.CloudFormationName.unsafeApply("DynamoDB")), cloudTrailEventSource = Some("dynamodb.amazonaws.com"), docId = None, endpointPrefix = Some("dynamodb")),
    smithy.api.XmlNamespace(uri = smithy.api.NonEmptyString.unsafeApply("http://dynamodb.amazonaws.com/doc/2012-08-10/"), prefix = None),
...
object DynamoDBOperation {
  ...
  object ListTables extends smithy4s.Endpoint[DynamoDBOperation,ListTablesInput, DynamoDBOperation.ListTablesError, ListTablesOutput, Nothing, Nothing] {
    val schema: OperationSchema[ListTablesInput, DynamoDBOperation.ListTablesError, ListTablesOutput, Nothing, Nothing] = Schema.operation(ShapeId("com.amazonaws.dynamodb", "ListTables"))
      ...
      .withHints(aws.api.ClientDiscoveredEndpoint(required = false), smithy.api.Documentation("<p>Returns an array of table names associated with the current account and endpoint. The output\n      from <code>ListTables</code> is paginated, with each page returning a maximum of 100 table\n      names.</p>"), smithy.api.Paginated(inputToken = Some(smithy.api.NonEmptyString.unsafeApply("ExclusiveStartTableName")), outputToken = Some(smithy.api.NonEmptyString.unsafeApply("LastEvaluatedTableName")), items = Some(smithy.api.NonEmptyString.unsafeApply("TableNames")), pageSize = Some(smithy.api.NonEmptyString.unsafeApply("Limit"))))
