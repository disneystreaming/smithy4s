...
object ListTablesInput extends ShapeTag.Companion[ListTablesInput] {
  ...
  val hints: Hints = Hints(
    Hints.Binding.DynamicBinding(ShapeId("smithy.api", "documentation"), smithy4s.Document.fromString("<p>Represents the input of a <code>ListTables</code> operation.</p>")),
  ).lazily
  ...
  implicit val schema: Schema[ListTablesInput] = struct(
    TableName.schema.optional[ListTablesInput]("ExclusiveStartTableName", _.exclusiveStartTableName).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "documentation"), smithy4s.Document.fromString("<p>The first table name that this operation will evaluate. Use the value that was returned for\n        <code>LastEvaluatedTableName</code> in a previous operation, so that you can obtain the next page\n      of results.</p>"))),
    ListTablesInputLimit.schema.optional[ListTablesInput]("Limit", _.limit).addHints(Hints.Binding.DynamicBinding(ShapeId("smithy.api", "documentation"), smithy4s.Document.fromString("<p>A maximum number of table names to return. If this parameter is not specified, the limit is 100.</p>"))),
