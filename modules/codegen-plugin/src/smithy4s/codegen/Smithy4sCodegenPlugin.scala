...
object Smithy4sCodegenPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val smithy4sGeneratedSmithyFilesDir =
      settingKey[File](
        "Output directory for generated smithy files"
      )
  ...
  def defaultSettings(config: Configuration) = Seq(
    ...
    config / smithy4sGeneratedSmithyFilesDir := {
      ...
      val cached = Tracked.inputChanged[String, Seq[File]](
        ...
        val lastOutput = Tracked.lastOutput[Boolean, Seq[File]](
          ...
          if (changed || prevResult.isEmpty) {
            val file = (config / smithy4sGeneratedSmithyFilesDir).value
            IO.write(
              file,
              s"""$$version: "2"
