...
private[codegen] object CodegenImpl { self =>
  ...
  private[internals] def generate(
    ...
    val alreadyGenerated: Set[String] = {
      val allGenerated: Seq[String] = CodegenRecord
      ...
      if (allGeneratedSet.size != allGenerated.size) {
        // There are duplicates. Find them and throw an Exception
        val duplicates = allGenerated
          .groupBy(s => s)
          .collect { case (s, l) if l.size > 1 => s }
          .toSeq
          .sorted
        throw new IllegalStateException(
          s"Multiple artifact manifests cannot contain generated code for the same namespace: ${duplicates
            .mkString(", ")}"
        )
      }
