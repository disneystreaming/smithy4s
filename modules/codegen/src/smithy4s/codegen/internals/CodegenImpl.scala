...
private[codegen] object CodegenImpl { self =>
  ...
  private[internals] def generate(
    ...
    val alreadyGenerated: Set[String] = {
      val allGenerated:List[String] = CodegenRecord
        .recordsFromModel(model)
        .flatMap { r =>
          r.namespaces
        }
      //if there are any duplicates
      //existing code is O(n^2) time but memory efficient
      //existing code returns a set, so take advantage of that
      //if the set is the same size as the list there are no duplicates
      //otherwise it's OK to use memory for a good error message
      for (g <- allGenerated) {
        if (allGenerated.count(_ == g) > 1) {
          
          //For 1338 throw an exception that lists the duplicates
