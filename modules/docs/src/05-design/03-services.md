...
* It will in handy for the implementation of some pagination-aware interpreters
...
NB: at the time of writing this, Smithy4s does not have any streaming-aware interpreter implemented. But streaming is such a fundamental notion in remote interactions, and we had to devise a plan to ensure that third party could decide to implement interpreters without waiting.
...
The goal of Smithy4s is to allow users to derive client stubs and routers in various protocols, by running the generated code (or instances of generated interfaces) in some one-liner functions. To that end, Smithy4s surfaces a number of abstractions (such as `smithy4s.schema.Schema`) that allow for the implementation of (very) polymorphic interpreters. Thse interpreters operate on the generated code, which reflects what the user defines in their smithy Specs.
