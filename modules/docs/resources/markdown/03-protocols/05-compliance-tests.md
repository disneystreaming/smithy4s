...
_Note: the following code and the `compliance-tests` module do not depend on a specific test framework. If you want to hook it into your test framework, it is easy to do so but it's outside the scope of this document. Refer to [this example](@GITHUB_BRANCH_URL@modules/tests/src/smithy4s/tests/ProtocolComplianceSuite.scala) to see how we did it for `Weaver` in this project._
