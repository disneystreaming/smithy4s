...
Here we will show an example using the `cats.Hash` typeclass. Ready-made `CachedSchemaCompiler`s for `cats.Show` and `cats.Hash` are available out of the box in the `smithy4s-cats` module.
