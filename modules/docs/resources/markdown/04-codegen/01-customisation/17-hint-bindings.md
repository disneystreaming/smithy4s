...
```scala
val hints = Hints(Hints.Binding.DynamicBinding(ShapeId("alloy", "Foo"), smithy4s.Document.obj("a" -> Document.fromString("test"), "b" -> Document.fromInt(2))))
```
...
```scala
val hints = Hints(Hints.Binding.DynamicBinding(ShapeId("alloy", "Bar"), smithy4s.Document.obj()))
```
...
If you wish to still use static hint bindings, there is a configuration option you can set with the `smithy4sRenderDynamicHintBindings` Smithy metadata field:

```smithy
$version: "2"

// defaults to true, setting to false will enable static hint bindings
metadata smithy4sRenderDynamicHintBindings = false
```
